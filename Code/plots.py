...
plt.tight_layout()
# pis = np.linspace(0.0001, 1, num=5)
# mean_MLMC = np.zeros(len(pis))
# std_MLMC = np.zeros(len(pis))


# for i, p in enumerate(pis):
#     mean_MLMC[i], std_MLMC[i] = MLMC(w0, v0, p, r, kappa, theta, xi, sigma, rho, T, N, epsilon, True)[:2]  

# plt.figure(dpi=300)
# plt.xlabel(r'Standard Deviation', fontsize=15, fontweight='bold')
# plt.ylabel(r'Expected Value', fontsize=15, fontweight='bold')
# plt.plot(std_MLMC, mean_MLMC,'-o', color = colors[3])
